        ...
        <section className="flex flex-col gap-4 md:gap-6 bg-white border border-neutral-base-100 p-4 md:p-8 rounded-xl md:rounded-[32px] shadow-sm">
            <div className="flex items-center justify-between border-b border-neutral-base-50 pb-4 md:pb-6">
                ...
                {remainingBill === 0 && useWallet && (
                    <span className="text-[8px] md:text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 md:px-3 py-1 rounded-full border border-emerald-100 uppercase tracking-widest">
                        Lunas via Wallet
                    </span>
                )}
            </div>
                ...
                <div className="flex flex-col gap-4 md:gap-6">
                    {isLoadingPayments ? (
                        <div className="flex flex-col items-center justify-center py-8 md:py-10 gap-3 md:gap-4">
                            ...
                            <p className="text-[10px] md:text-[12px] font-bold text-neutral-base-400 uppercase tracking-widest">Memuat Metode Pembayaran...</p>
                        </div>
                    ) : (
                                        ...
                                        <div className="flex items-center gap-3 md:gap-4">
                                            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 transition-all font-black text-[10px] ${paymentMethod === method.namaBank ? "bg-amber-100 text-amber-800" : "bg-neutral-base-50 text-neutral-base-400 group-hover:bg-amber-50 group-hover:text-amber-800"}`}>
                                                {method.logoBank ? (
                                                    ...
                                                    <Landmark className="w-4 h-4 md:w-5 md:h-5 opacity-50" />
                                                )}
                                            </div>
                                            <div className="flex flex-col items-start text-left">
                                                ...
                                                {isDisabled ? (
                                                    <p className="text-[8px] md:text-[9px] font-bold text-red-500 uppercase tracking-widest mt-0.5 md:mt-1">Min. Transfer Rp 10.000</p>
                                                ) : (
                                                    <p className="text-[8px] md:text-[9px] font-bold text-neutral-base-400 uppercase tracking-widest mt-0.5 md:mt-1">{method.namaPemilik}</p>
                                                )}
                                            </div>
                                        </div>
